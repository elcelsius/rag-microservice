...
echo "[treinar_ia_gpu] data=$DATA_DIR out=$OUT_DIR emb=$EMB exts=$EXTS loaders=$LOADERS_DIR"
docker-compose -f docker-compose.gpu.yml build ai_etl
docker-compose -f docker-compose.gpu.yml run --rm -e EMBEDDINGS_MODEL="$EMB" ai_etl \
