...
3. **Busca lexical**: ranking inicial com RapidFuzz sobre todos os documentos do `vectorstore.docstore`, ajudando a detectar nomes próprios e consultas muito específicas.
...
- `api.py`: servidor Flask com endpoints `/agent/ask`, `/query`, `/healthz` e `/metrics`, além da inicialização de embeddings e FAISS.
...
- `web_ui/`: assets estáticos (HTML/JS/CSS) servidos pelo container Nginx (`ai_web_ui`).
...
- `POST /agent/ask`: fluxo completo com LangGraph e suporte a histórico de mensagens.
- `POST /query`: pipeline RAG direto (sem agente), útil para testes e comparações.
